...
export async function ComprasListSection({ searchParams }: ComprasListSectionProps) {
	...
	const filtered = (compras ?? []).filter((compra) =>
		[compra.codcompra, compra.fornecedor, compra.modelo, compra.serie, compra.numero_nota, compra.condicao_pagamento, compra.status]
				...
				<div className="overflow-x-auto">
					<table className="min-w-full border-separate border-spacing-y-3">
						...
						<tbody>{filtered.map((compra) => (
							<tr key={compra.codcompra} className="bg-neutral-50">
								...
								<td className="rounded-r-xl px-4 py-3">
									<div className="flex items-center justify-end gap-2">
										<Button asChild variant="outline" size="icon" className="rounded-xl border-neutral-300" title="Visualizar compra">
											<Link href={`/cadastro/compras?edit=${compra.codcompra}`} aria-label="Visualizar compra">
												<Edit className="h-4 w-4" aria-hidden="true" />
											</Link>
										</Button>
										<CancelCompraForm codcompra={compra.codcompra} disabled={compra.status === "CANCELADA"} />
									</div>
								</td>
							</tr>
						))}</tbody>

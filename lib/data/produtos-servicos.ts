...
export async function listarProdutosComCategorias() {
	const [
		...
		queryRows(
			"select codproduto, produto, codcategoria, codmarca, codunidade_medida, valor, quantidade_estoque, valor_desconto, imagem_url, ativo, data_cadastro as data_criacao, data_ult_alteracao as data_atualizacao from public.produtos order by produto asc"
		),
	]);
...
export async function buscarProdutoPorId(codproduto: number) {
	return queryMaybeSingle(
		"select codproduto, produto, codcategoria, codmarca, codunidade_medida, valor, quantidade_estoque, valor_desconto, imagem_url, ativo, data_cadastro as data_criacao, data_ult_alteracao as data_atualizacao from public.produtos where codproduto = $1",
		[codproduto]
	);
}
...
export async function listarServicosComCategorias() {
	const [{ data: categorias }, { data: servicos, error }] = await Promise.all([
		queryRows("select codcategoria, categoria from public.categorias order by categoria asc"),
		queryRows(
			"select codservico, servico, codcategoria, duracao_minutos, valor, valor_desconto, imagem_url, ativo, data_cadastro as data_criacao, data_ult_alteracao as data_atualizacao from public.servicos order by servico asc"
		),
	]);
...
export async function buscarServicoPorId(codservico: number) {
	return queryMaybeSingle(
		"select codservico, servico, codcategoria, duracao_minutos, valor, valor_desconto, imagem_url, ativo, data_cadastro as data_criacao, data_ult_alteracao as data_atualizacao from public.servicos where codservico = $1",
		[codservico]
	);
}

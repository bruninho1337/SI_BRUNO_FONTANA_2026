...
export async function listarServicosComCategorias() {
	const [{ data: categorias }, { data: servicos, error }] = await Promise.all([
		queryRows("select codcategoria, categoria from public.categorias order by categoria asc"),
		queryRows(
			"select codservico, servico, codcategoria, duracao_minutos, valor, valor_desconto, ativo, data_cadastro as data_criacao, data_ult_alteracao as data_atualizacao from public.servicos order by servico asc"
		),
	]);
...
export async function buscarServicoPorId(codservico: number) {
	return queryMaybeSingle(
		"select codservico, servico, codcategoria, duracao_minutos, valor, valor_desconto, ativo, data_cadastro as data_criacao, data_ult_alteracao as data_atualizacao from public.servicos where codservico = $1",
		[codservico]
	);
}

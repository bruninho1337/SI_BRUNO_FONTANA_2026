create table if not exists public.compras (
	...
	constraint compras_pkey primary key (modelo, serie, numero_nota, codfornecedor),
...
create table if not exists public.compras_itens (
	...
	valor_desconto numeric(14,2) not null default 0 check (valor_desconto >= 0),
	valor_total numeric(14,2) not null check (valor_total >= 0),
	constraint compras_itens_pkey primary key (modelo, serie, numero_nota, codfornecedor, num_item),
	constraint compras_itens_compra_fkey foreign key (modelo, serie, numero_nota, codfornecedor)
		references public.compras(modelo, serie, numero_nota, codfornecedor) on delete cascade,
	constraint compras_itens_produto_fkey foreign key (codproduto)
		references public.produtos(codproduto),
	constraint compras_itens_produto_unique unique (modelo, serie, numero_nota, codfornecedor, codproduto),

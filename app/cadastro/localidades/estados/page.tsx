		...
		<div className="min-h-screen bg-neutral-100 p-6 md:p-8">
			<div className="mx-auto flex max-w-7xl flex-col gap-6 lg:flex-row">
				...
				<div className="flex-1 space-y-8">
					...
					<Suspense fallback={<LocalidadesSectionFallback title="Dados do Estado" />}>
						<EstadoFormSection searchParams={searchParams} />
					</Suspense>
					<Suspense fallback={<LocalidadesSectionFallback title="Estados cadastrados" />}>
						<EstadosListSection />
					</Suspense>
				</div>
			</div>
...
async function EstadoFormSection({
	searchParams,
}: {
	...
	return (
		<div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
			...
			{paisesError ? (
				<p className="mb-4 text-sm text-red-600">Erro ao carregar países: {paisesError.message}</p>
			) : null}

			<form action={createEstadoAction} className="space-y-4">
				<div className="flex flex-col gap-2">
					<Label htmlFor="uf-estado" className="text-sm text-neutral-800">UF:</Label>
					<Input id="uf-estado" name="uf" type="text" placeholder="Ex: SP" className="h-11 rounded-xl border-neutral-300 bg-white px-4 text-neutral-900" />
				</div>

				<div className="flex flex-col gap-2">
					<Label htmlFor="nome-estado" className="text-sm text-neutral-800">Estado:</Label>
					<Input id="nome-estado" name="estado" type="text" placeholder="Ex: São Paulo" className="h-11 rounded-xl border-neutral-300 bg-white px-4 text-neutral-900" />
				</div>
...
async function EstadosListSection() {
	const supabase = await createClient();
	const [{ data: paises }, { data: estados, error: estadosError }] = await Promise.all([
		supabase.from("paises").select("codpais, pais"),
		supabase.from("estados").select("codestado, estado, uf, codpais, ativo").order("estado", { ascending: true }),
	]);
	...
	return (
		<div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
			...
			{estadosError ? (
				<p className="text-sm text-red-600">Erro ao carregar estados: {estadosError.message}</p>

		...
		<div className="min-h-screen bg-neutral-100 p-6 md:p-8">
			<div className="mx-auto flex max-w-7xl flex-col gap-6 lg:flex-row">
				...
				<div className="flex-1 space-y-8">
					...
					<Suspense fallback={<LocalidadesSectionFallback title="Dados da Cidade" />}>
						<CidadeFormSection searchParams={searchParams} />
					</Suspense>
					<Suspense fallback={<LocalidadesSectionFallback title="Cidades cadastradas" />}>
						<CidadesListSection />
					</Suspense>
				</div>
			</div>
...
async function CidadeFormSection({
	searchParams,
}: {
	...
	return (
		<div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
			...
			{estadosError ? (
				<p className="mb-4 text-sm text-red-600">Erro ao carregar estados: {estadosError.message}</p>
			) : null}

			<form action={createCidadeAction} className="space-y-4">
				<div className="flex flex-col gap-2">
					<Label htmlFor="nome-cidade" className="text-sm text-neutral-800">Cidade:</Label>
					<Input id="nome-cidade" name="cidade" type="text" placeholder="Ex: Campinas" className="h-11 rounded-xl border-neutral-300 bg-white px-4 text-neutral-900" />
				</div>
...
async function CidadesListSection() {
	const supabase = await createClient();
	const [{ data: estados }, { data: cidades, error: cidadesError }] = await Promise.all([
		supabase.from("estados").select("codestado, estado, uf"),
		supabase.from("cidades").select("codcidade, cidade, codest, ativo").order("cidade", { ascending: true }),
	]);
	...
	return (
		<div className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
			...
			{cidadesError ? (
				<p className="text-sm text-red-600">Erro ao carregar cidades: {cidadesError.message}</p>
